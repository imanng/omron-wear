...
/**
 * Parsed Latest data (characteristic 0x3001) from OMRON 2JCIE-BL01. Units per Communication
 * Interface Manual Table 8.
 */
...
fun uvIndexToLabel(index: Double): String =
        when {
            index < 0 -> "$index (—)"
            index <= 2.0 -> "$index (Low)"
            index <= 5.0 -> "$index (Moderate)"
            index <= 7.0 -> "$index (High)"
            index <= 10.0 -> "$index (Very High)"
            else -> "$index (Extreme)"
        }

/** Discomfort index bands */
fun discomfortToLabel(index: Double): String =
        when {
            index < 55 -> "$index (Cold)"
            index < 60 -> "$index (Cool)"
            index < 65 -> "$index (Comfortable)"
            index < 70 -> "$index (Slightly warm)"
            index < 75 -> "$index (Warm)"
            index < 80 -> "$index (Hot)"
            else -> "$index (Very hot)"
        }
...
fun heatstrokeToLabel(wbgtC: Double): String =
        when {
            wbgtC < 25 -> " Low"
            wbgtC < 28 -> "$wbgtC (Caution)"
            wbgtC < 31 -> "$wbgtC (Warning)"
            else -> "$wbgtC (Danger)"
        }
